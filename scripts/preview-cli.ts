...
const code=await main(process.argv.slice(2),{chooseMode,setup:mode=>setupDisplayMode({mode,packageRoot,applicationSupport}),runChild,status,uninstall:()=>runChild(join(packageRoot,'app','install.js'),['uninstall']),write:value=>console.log(value),runtimeEntry:join(packageRoot,'app','runtime.ts'),studioEntry:join(packageRoot,'app','studio.js'),version:packageVersion});
