...
const fixture=()=>{
  ...
  for(const file of ['README.md','DEVELOPMENT.md','packaging/install.sh','packaging/uninstall.sh','packaging/bin/streamhub','packaging/runtime.ts','packages/actions/composite.ts','packages/core/src/index.ts','packages/host/native/session-monitor.swift','packages/host/src/main.ts','packages/presentation/backend.ts','packages/streamdeck/hid.ts','packages/studio/document.ts','packages/editor/web/index.html','packages/editor/web/style.css','packages/editor/web/icon-library.css','packages/editor/web/display-settings.css'])put(repositoryRoot,file);
  ...
  const options:PackageOptions={repositoryRoot,outputRoot,tempRoot,platform:'darwin',arch:'arm64',gitCommit:'b'.repeat(40),builtAt:'2026-09-11T00:00:00.000Z',preparePlugin:async()=>{},buildApplications:async({packageRoot})=>{put(packageRoot,'app/cli.js','cli');put(packageRoot,'app/install.js','installer');put(packageRoot,'app/studio.js','studio');put(packageRoot,'share/studio/app.js','browser');},installDependencies:async({appRoot})=>{put(appRoot,'node_modules/sharp/index.js','dependency');put(appRoot,'node_modules/sharp/index.test.js','test');put(appRoot,'node_modules/sharp/index.js.map','map');},archive:async({archivePath})=>put(dirname(archivePath),archivePath.split('/').at(-1)!,'archive')};
  return{options};
};

test('assembler ships daemon-capable CLI and installer with an unbundled Runtime but no private, machine, or development files',async()=>{
  const {options}=fixture(),result=await assemblePreview(options),files=relativeFiles(result.root);
  expect(files).toEqual(expect.arrayContaining(['README.md','DEVELOPMENT.md','manifest.json','SHA256SUMS','install.sh','uninstall.sh','bin/streamhub','app/cli.js','app/install.js','app/runtime.ts','app/studio.js','app/packages/actions/composite.ts','app/packages/core/src/index.ts','app/packages/host/native/session-monitor.swift','app/packages/host/src/main.ts','app/packages/presentation/backend.ts','app/packages/streamdeck/hid.ts','app/packages/studio/document.ts','share/studio/index.html','share/studio/app.js','share/streamdeck-plugin/com.streamhub.studio.sdPlugin/manifest.json']));
  expect(files).not.toContain('app/runtime.js');
  expect(files.some(path=>/(^|\/)(\.streamhub|\.git|logs|tests?|__tests__)(\/|$)|\.(?:test\.|map$)|(?:config|token)(?:\.|$)|\.plist$|^\//.test(path))).toBe(false);
});
