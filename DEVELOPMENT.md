# Streamhub Development

Streamhub의 소스 빌드, 구조, 프로토콜, 시뮬레이터와 실기기 검증을 위한 개발자 문서입니다. 배포본 사용법은 [README](README.md)를 참고하세요.

저장소에서 생성되는 `.streamhub`는 개발 전용 로컬 상태이며 릴리스 입력에 포함하면 안 됩니다. 패키지 조립기는 허용 목록만 복사하고 이 디렉터리, 로그, 테스트와 소스맵을 거부합니다.
...
Developer Preview 패키지는 전체 검증 후 생성합니다.
...
- **아이콘 라이브러리**에서 Stream Deck 앱에 설치된 아이콘팩을 이름과 태그로 검색해 사용할 수 있습니다. 선택한 아이콘은 Streamhub 자산으로 복사되며 SVG와 래스터 이미지는 PNG로 정규화되고, 애니메이션 GIF·WEBP는 현재 첫 프레임을 사용합니다.
...
버튼을 선택한 뒤 동작 목록의 **여러 동작**, **토글**, **두 번 누르기**, **길게 누르기**를 선택하거나 오른쪽의 **누르는 방식**에서 직접 구성합니다.

- 여러 동작은 최대 16단계이며 차례 실행 또는 동시 실행을 고릅니다. 기다리기는 10–30,000ms 범위의 차례 실행에서만 사용할 수 있습니다.
- 페이지 이동은 차례 실행의 마지막 단계에만 둘 수 있습니다. 동시 실행이나 토글 중간에서 페이지를 바꾸지 않습니다.
- 두 번 누르기 간격은 150–750ms, 길게 누르기 기준은 300–2,000ms이며 길게 기준이 더 커야 합니다.
- 토글 값은 Studio 문서와 분리된 `.streamhub/button-state.json`에 저장되고, 선택한 전환 동작이 성공한 뒤에만 바뀝니다.
- 페이지/프레임 변경, 잠금, 플러그인 연결 종료 중인 대기·실행 동작은 취소하며 자동 재시도하지 않습니다.

[M2 자동 검증과 실기기 체크리스트](docs/validation/studio-advanced-actions.md).
