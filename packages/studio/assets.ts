...
export class VisualAssetStore{
  ...
  async put(input:Uint8Array):Promise<string>{
    ...
    if(!metadata.format||!['png','jpeg','webp','gif','tiff','avif','heif'].includes(metadata.format))throw new Error('Unsupported image format');
    if(!metadata.width||!metadata.height||metadata.width*metadata.height>MAX_PIXELS)throw new Error('Asset exceeds pixel limit');
    const bytes=await sharp(input,{animated:false,limitInputPixels:MAX_PIXELS}).rotate().png({compressionLevel:9,adaptiveFiltering:false}).toBuffer();
