...
describe('VisualAssetStore',()=>{
  ...
  test('rasterizes a bounded SVG icon into the same durable PNG format',async()=>{const s=store(),svg=Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="144" height="144"><circle cx="72" cy="72" r="60" fill="#ff3366"/></svg>'),id=await s.put(svg);expect(await sharp(await s.read(id)).metadata()).toMatchObject({format:'png',width:144,height:144});});
