...
test('Studio combines a saved mode with the unchanged active Runtime owner',async()=>{const runtime=Bun.serve({hostname:'127.0.0.1',port:0,fetch:request=>new URL(request.url).pathname==='/v1/state'?Response.json({display:{configuredMode:'plugin',activeMode:'plugin',state:'ready',restartRequired:false}}):Response.json({error:'Not found'},{status:404})});try{const plugin={port:31417,tokenFile:'/private/plugin-token'},config={...baseConfig(),port:runtime.port!,display:{mode:'plugin' as const,plugin}};const {url,bootstrap}=await setup({},config);expect(bootstrap.display).toMatchObject({configuredMode:'plugin',activeMode:'plugin',restartRequired:false});const response=await fetch(`${url}/api/display-mode`,{method:'POST',headers:{'X-Streamhub-Editor':bootstrap.token,'Content-Type':'application/json'},body:JSON.stringify({mode:'hid',expectedVersion:bootstrap.configVersion})}),saved=await response.json() as any;expect(saved.display).toMatchObject({configuredMode:'hid',activeMode:'plugin',state:'ready',restartRequired:true});}finally{await runtime.stop(true);}});
