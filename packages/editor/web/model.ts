...
export class StudioModel{
  ...
  private change(mutator:(draft:StudioDocument)=>void){this.past.push(structuredClone(this.document));if(this.past.length>100)this.past.shift();this.future=[];const draft=structuredClone(this.document);mutator(draft);this.document=validateStudioDocument(draft);}
  ...
  addPage(){let n=2;while(this.document.pages.some(page=>page.id===`page-${n}`))n++;const id=`page-${n}`;this.change(document=>document.pages.push({id,title:`페이지 ${n}`}));this.selectedPageId=id;}
  ...
  undo(){const previous=this.past.pop();if(previous){this.future.push(structuredClone(this.document));this.document=previous;}}
  redo(){const next=this.future.pop();if(next){this.past.push(structuredClone(this.document));this.document=next;}}
