...
export class StudioModel{
  ...
  addPage(){const existing=new Set(this.document.pages.map(page=>page.id)),next=editing.addPage(this.document,this.context);this.replace(next);this.selectedPageId=next.pages.find(page=>!existing.has(page.id))!.id;}
  renamePage(pageId:string,title:string){this.replace(editing.renamePage(this.document,pageId,title,this.context));this.selectedPageId=pageId;}
  duplicatePage(pageId:string){const existing=new Set(this.document.pages.map(page=>page.id)),next=editing.duplicatePage(this.document,pageId,this.context);this.replace(next);this.selectedPageId=next.pages.find(page=>!existing.has(page.id))!.id;}
  movePage(pageId:string,offset:-1|1){this.replace(editing.movePage(this.document,pageId,offset,this.context));this.selectedPageId=pageId;}
  setDefaultPage(pageId:string){this.replace(editing.setDefaultPage(this.document,pageId,this.context));}
  deletePage(pageId:string,replacementPageId?:string){const index=this.document.pages.findIndex(page=>page.id===pageId),next=editing.deletePage(this.document,pageId,replacementPageId,this.context);this.replace(next);if(this.selectedPageId===pageId)this.selectedPageId=next.pages[Math.min(index,next.pages.length-1)]!.id;}
  ...
  copyButton(){this.clipboard=buttonEditing.copyButton(this.document,this.selectedPageId,this.selectedKey,this.context);}
  pasteButton(){if(!this.clipboard)throw new Error('복사한 버튼이 없습니다.');return this.replace(buttonEditing.pasteButton(this.document,this.selectedPageId,this.selectedKey,this.clipboard,this.context));}
  duplicateButton(targetIndex?:number){const page=this.document.pages.find(page=>page.id===this.selectedPageId)!,target=targetIndex??Array.from({length:15},(_,index)=>index).find(index=>!page.buttons?.some(button=>button.index===index));if(target===undefined)throw new Error('빈 키가 없습니다.');const changed=this.replace(buttonEditing.duplicateButton(this.document,this.selectedPageId,this.selectedKey,target,this.context));this.selectedKey=target;return changed;}
  moveButton(from:{pageId:string;index:number},to:{pageId:string;index:number},swap=false){const changed=this.replace(buttonEditing.moveButton(this.document,from,to,{swap},this.context));this.selectedPageId=to.pageId;this.selectedKey=to.index;return changed;}
  removeButton(index=this.selectedKey){const changed=this.replace(buttonEditing.removeButton(this.document,this.selectedPageId,index,this.context));this.selectedKey=index;return changed;}
