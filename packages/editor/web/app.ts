...
let studio:StudioState,standby=false,query='';
const currentPage=()=>studio.model.document.pages.find(page=>page.id===studio.model.selectedPageId)!;
const selectedButton=()=>currentPage().buttons?.find(button=>button.index===studio.model.selectedKey);
...
function chooseAction(type:ButtonAction['type'],index=studio.model.selectedKey){
  ...
  if(selectedButton()&&index===studio.model.selectedKey&&!confirm('이 키의 기존 버튼을 바꿀까요?'))return;
...
function render(){
  ...
  renderCanvas($('canvas'),studio.model,studio.geometry,{standby,select:index=>{studio.model.selectKey(index);render();},drop:(type,index)=>chooseAction(type,index)});
...
$('undo').onclick=()=>{studio.model.undo();render();};$('redo').onclick=()=>{studio.model.redo();render();};
