...
function visual(key:HTMLButtonElement,button:ButtonDefinition):void{
  ...
  const badge=document.createElement('i');badge.textContent=buttonBadge(button);key.append(badge);
}
export function buttonBadge(button:ButtonDefinition):'PAGE'|'GITHUB'|'ACTION'{const types=actionsInBehavior(button.behavior).map(action=>action.type);return types.includes('github-pipeline')?'GITHUB':types.some(type=>['go-to-page','previous-page','next-page','page-indicator','resume-auto-page'].includes(type))?'PAGE':'ACTION';}
