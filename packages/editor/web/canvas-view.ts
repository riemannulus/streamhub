import type {ButtonAction,ButtonDefinition,StudioAppearance} from '../../studio/document';
...
function visual(key:HTMLButtonElement,button:ButtonDefinition):void{
  ...
  const badge=document.createElement('i');badge.textContent=button.action.type==='go-to-page'||button.action.type==='previous-page'||button.action.type==='next-page'||button.action.type==='page-indicator'||button.action.type==='resume-auto-page'?'PAGE':'ACTION';key.append(badge);
}
export function renderCanvas(canvas:HTMLElement,model:StudioModel,geometry:Geometry,options:{standby:boolean;select(index:number):void;drop(type:ButtonAction['type'],index:number):void}):void{
