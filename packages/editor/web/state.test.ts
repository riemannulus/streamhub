...
test('an app action can start with its native icon and app name',()=>{
  const iconAssetId='d'.repeat(64);
  const button=createButtonForAction('open-app',0,{
    pageId:'home',
    app:{id:'bundle:org.mozilla.firefox',name:'Firefox',bundleId:'org.mozilla.firefox',path:'/Applications/Firefox.app'},
    iconAssetId,
  } as never);
...
test('stored browser fetch keeps the global receiver for later icon requests',async()=>{
  const document=defaultStudioDocument(),packId='a'.repeat(64);
  const request=function(this:unknown,input:string|URL|Request){
    const url=String(input);
    if(url==='/api/icon-packs'&&this!==globalThis)throw new TypeError("Failed to execute 'fetch' on 'Window': Illegal invocation");
    if(url==='/api/bootstrap')return Promise.resolve(Response.json({token:'t',snapshot:{document,version:'v1'},runtimeStatus:{connected:false},geometry:{x:[11,108,205,302,399],y:[5,102,199]}}));
    if(url.includes('/catalog/'))return Promise.resolve(Response.json([]));
    if(url==='/api/icon-packs')return Promise.resolve(Response.json([{id:packId,name:'Media',version:'1.0.0',author:'Example',iconCount:0,hasLicense:false}]));
