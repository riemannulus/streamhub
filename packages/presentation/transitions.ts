import sharp from 'sharp';import type {TransitionSpec} from '../studio/document';import type {FramePlan} from './playback';import {extractKeyPngs} from './geometry';
const uri=(b:Uint8Array)=>`data:image/png;base64,${Buffer.from(b).toString('base64')}`;
export class TransitionCompiler{
  constructor(private readonly frameIntervalMs=40){}
  async compile(from:Uint8Array,to:Uint8Array,spec:TransitionSpec,generation:string|number):Promise<FramePlan>{
    const duration=spec.type==='none'?0:Math.min(500,Math.max(0,spec.durationMs)),count=duration===0?1:Math.min(12,Math.max(2,Math.ceil(duration/this.frameIntervalMs)));
