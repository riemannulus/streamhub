...
export class DeckVisualRenderer{
  ...
  private async renderAppearance(appearance:StudioAppearance|undefined,deckPage:DeckPage,page:StudioPage|undefined,assets:AssetReader):Promise<DeckCanvas>{
    ...
    for(const key of deckPage.keys){const fixed=page?.buttons?.find(button=>button.index===key.index);const opacity=fixed?.appearance?.opacity??(fixed?1:key.type==='empty'?0:1);if(opacity===0)continue;let input:Buffer;if(fixed?.appearance?.iconAssetId)input=await sharp(await assets.read(fixed.appearance.iconAssetId)).resize(72,72,{fit:'contain'}).png().toBuffer();else input=await sharp(await renderKey(key,deckPage),{raw:{width:72,height:72,channels:3}}).png().toBuffer();if(opacity<1)input=await sharp(input).ensureAlpha().linear([1,1,1,opacity],[0,0,0,0]).png().toBuffer();overlays.push({input,left:keyViewport(key.index).left,top:keyViewport(key.index).top});}
