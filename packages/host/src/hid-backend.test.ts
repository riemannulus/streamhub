...
test('HID standby writes the exact Studio surface before closing without a logo reset',async()=>{
  const order:string[]=[],last:Buffer[]=[];const backend=await startHidBackend({events:{key:()=>{},ready:()=>{}},connect:async()=>new HidDisplay({fillKeyBuffer:async(index,bytes)=>{last[index]=Buffer.from(bytes);if(index===14)order.push('frame');},resetToLogo:async()=>{order.push('logo');},close:async()=>{order.push('close');}})});
  await backend.present(await backend.prepare(request('g-lock','standby')));
  expect(order).toEqual(['frame','close']);expect([...last[0]!.subarray(0,3)]).toEqual([255,0,0]);expect(backend.status()).toMatchObject({connected:false});
  await backend.stop();
});
