...
표시되는 `http://127.0.0.1:31416`을 브라우저에서 엽니다. 장치 없이 샘플 신호로 동작하며 실제 Stream Deck이나 신호 DB를 열지 않습니다. 기존 페이지·키 렌더러·페이드 전송 로직의 RGB 출력을 그대로 캔버스에 표시합니다.

- 페이지를 추가하고 제목, 기본 페이지, 앱 조건, 복수 소스·중요도·신선도 필터를 편집합니다.
- 소스별 키 영역을 만들고 현재 필터에 해당하는 신호 수와 일반·영역별 용량을 확인합니다.
- 페이지 복제·위아래 정렬, 템플릿 생성, 실행 취소·다시 실행을 지원합니다. 고정 키를 복사·붙여넣고 색상과 내장 아이콘을 선택할 수 있습니다.
- **초안 검증**은 저장 전 구성을 공통 호스트 실행기로 검사하며, **검증 기록**에서 키별 출력을 재생합니다.
- 배치 편집 모드에서 고정 버튼을 옮기거나 선택한 키의 표시·페이지 이동·자동 복귀 기능을 설정합니다.
- 시뮬레이션 모드에서 키를 눌러 탐색하고 신호 추가·갱신·삭제·소스 stale 전환, 가상 활성 앱, 잠금, 키별 전송 지연을 재현합니다.
- 페이드 유무와 시간을 조정하고 구성 파일을 가져오거나 내보낼 수 있습니다.
- 미리보기는 초안을 즉시 사용합니다. **설정 저장**을 눌러야 실제 설정 파일에 반영되며, 실행 중인 호스트는 재시작해야 적용됩니다. 저장은 기존 인증 토큰과 Stream Deck 활성화 여부를 보존합니다.

다른 편집기나 CLI에서 설정을 바꾼 경우 저장 충돌을 표시합니다. 최신 설정을 다시 읽기 전에 초안을 내보내 보관할 수 있습니다. `STREAMHUB_CONFIG`를 지정하면 해당 파일을 편집합니다. 편집기는 인증 토큰을 브라우저로 전달하지 않으며, 별도 루프백 서버와 임시 접근 토큰을 사용합니다. 기존 신호 API의 브라우저 접근 제한은 유지됩니다.
